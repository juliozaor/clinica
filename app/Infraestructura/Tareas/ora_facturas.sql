SELECT
    ...
    TRIM(e.CON_EMP_DESCRIPCIO) Empresa,
        DECODE(rpa.RPA_FOR_TIPOPROC, 'L', 'LABORATORIO','I', 'IMAGENES','A', 'PATOLOGIA','C', 'PROCEDIMIENTOS','D', 'CONSULTA','E', 'PROCEDIMIENTOS','F', 'PROCEDIMIENTOS','G', 'PROCEDIMIENTOS','J', 'PROCEDIMIENTOS','K', 'PROCEDIMIENTOS','M', 'PROCEDIMIENTOS','N', 'PROCEDIMIENTOS','O', 'PROCEDIMIENTOS','P', 'PROCEDIMIENTOS','Q', 'PROCEDIMIENTOS','R', 'PROCEDIMIENTOS','S', 'PROCEDIMIENTOS','T', 'PROCEDIMIENTOS','U', 'PROCEDIMIENTOS','X', 'PROCEDIMIENTOS','Y', 'PROCEDIMIENTOS',' ', 'CONSULTA') Ambito,
...
WHERE
        Trunc(rpa.rpa_for_fechatencion) >= TO_DATE('2023/01/01', 'yyyy/mm/dd')
        ...
        AND NOT EXISTS(SELECT 'x'
                                        ...
                                        WHERE grp.rpa_for_numerformu = rpa.rpa_for_numerformu
                                                        ...
                                                        AND grp.fecprc <= LAST_DAY(TO_DATE('2023/01/25', 'yyyy/mm/dd')) + 1)
        AND ( EXISTS(SELECT 'x'
